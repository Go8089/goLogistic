    ...
    <div className="mx-auto flex max-w-2xl justify-center px-4 py-12 sm:px-6 sm:py-20">
      <div className="w-full rounded-xl border border-gray-200 bg-white p-6 text-center sm:p-10">
        ...
        <p className="mt-3 text-sm leading-6 text-gray-500">
          ...
          Your transportation booking will now be confirmed.
        </p>

        <div className="mt-7 divide-y divide-gray-100 rounded-lg border border-gray-200 text-left">
          <Row
            label="Quote ID"
            value={state?.quoteId ?? "QT10001"}
          />

          <Row
            label="Amount Paid"
            value={state?.amount ?? "₹18,500"}
          />

          <Row
            label="Payment Method"
            value={state?.paymentMethod ?? "UPI"}
          />

          <Row
            label="Transaction ID"
            value={state?.transactionId ?? "TXN000000"}
          />
        </div>

        <Link
          to="/dashboard/shipments"
          className="mt-7 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-700"
        >
          View My Shipments
          <ArrowRight size={16} />
        </Link>
